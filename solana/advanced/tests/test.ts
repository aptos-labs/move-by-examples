...
describe("test", () => {
  ...
  it("Creates a todo list", async () => {
    ...
    const user1TodoListCounter = await program.account.userTodoListCounter
      ...
      .then((counter) => {
        expect(counter.owner.toBase58()).to.equal(user1.publicKey.toBase58());
        expect(counter.counter.toString()).to.eq("0");
        return counter;
      });
    ...
    await program.account.todoList.fetch(todoList1Pda).then((todoList) => {
      expect(todoList.owner.toBase58()).to.equal(user1.publicKey.toBase58());
      expect(todoList.todos.length).to.equal(0);
    });
    ...
    await program.account.todoList.fetch(todoList1Pda).then((todoList) => {
      ...
      expect(todo.content).to.equal("todo 1");
    ...
    await program.account.todoList.fetch(todoList1Pda).then((todoList) => {
      ...
      expect(completedTodo.content).to.equal("todo 1");
    ...
    await program.account.todoList.fetch(todoList1Pda).then((todoList) => {
      ...
      expect(completedTodo2.content).to.equal("todo 1");
      expect(completedTodo2.completed).to.equal(true);
    });
  });
});
